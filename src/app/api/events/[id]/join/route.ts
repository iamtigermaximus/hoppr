...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  if (event.maxAttendees) {
    const count = await prisma.eventParticipant.count({ where: { eventId: params.id } });
    if (count >= event.maxAttendees) {
      return NextResponse.json({ error: "Event is full" }, { status: 409 });
    }
  }

  await prisma.eventParticipant.create({ data: { userId, eventId: params.id } });

  // Auto-create chat room
  const existingRoom = await prisma.chatRoom.findUnique({ where: { eventId: params.id } });
  if (!existingRoom) {
    await prisma.chatRoom.create({ data: { eventId: params.id } });
  }
