        ...
        <Card key={event.id} onClick={() => router.push(`/events/${event.id}/chat`)}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <div style={{ color: "#a3a3a3", fontSize: "11px", marginTop: "2px" }}>{event.venueName} · {event.participants?.length || 0} participants</div>
            </div>
            {event.participants?.length > 0 && (
              <AvatarGroup users={event.participants.map((p: any) => ({ id: p.user.id, name: p.user.username, image: p.user.avatarUrl }))} max={4} size={26} />
            )}
          </div>
        </Card>
      ))}
