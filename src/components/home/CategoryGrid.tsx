...
import { House, Users, Heart, MusicNotes, Clock, Microphone, Pen, BeerBottle } from "@phosphor-icons/react";
...
export function CategoryGrid() {
  ...
  return (
    <div style={{ marginBottom: "18px", padding: "0 16px" }}>
      ...
      <Grid>
        {/* "All" option */}
        <CategoryCard onClick={() => router.push("/bars")}>
          <House size={22} color="#7c3aed" weight="regular" />
          <div style={{ color: "#7c3aed", fontSize: "10px", fontWeight: 600 }}>All</div>
        </CategoryCard>
        {CATEGORIES.map((cat) => {
          const IconComponent = iconMap[cat.icon] || Users;
          return (
            <CategoryCard key={cat.key} onClick={() => router.push(`/bars`)}>
